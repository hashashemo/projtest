...
function UserManagement() {
  ...
  const getFilteredData = () => {
    switch (activeTab) {
      case 'all': {
        // Pending first (oldest → newest), then active users (oldest → newest)
  ...
  // Unified columns for the "All" tab (handles both pending & active rows)
  const allColumns = [
    {
  ...
  // Columns for dedicated role tabs (active users only)
  const userColumns = [
    {
      key: 'entity_name', label: 'Organization',
      render: (val) => val || <span style={{ color: 'var(--text-muted)' }}>—</span>
  ...
  // Columns for the dedicated "Pending Requests" tab
  ...
  // In the "All" tab, delegate to the right renderer based on _isPending flag
  const renderAllActions = (row) =>
    row._isPending ? renderPendingActions(row) : renderUserActions(row)

  // Pick the right columns & actions for the active tab
  const activeColumns =
